...
class MyRoleCommand extends Command {
  ...
  async exec(message, args) {
    ...
    const parentRole = '807040777120251904';
    ...
    if (!args.roleName) {
      ...
      if (
        await this.client.db.customRoles.findOne({
      ...
      if (message.member.roles.cache.has(roles.nitroBoosterRole)) {
        if (
          ...
          await this.client.db.customRoles
            .create({
              roleOwner: message.member.id,
            })
            .then(async () => {
              await message.guild.roles
                ...
                .then(async (role) => {
                  ...
                  await this.client.db.customRoles.findOneAndUpdate(
                    filter,
                    update
                  );
